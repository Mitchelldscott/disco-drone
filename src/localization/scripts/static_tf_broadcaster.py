...
class IMU_Odometry():
	def __init__(self):
		...
		self.mag_sub = rospy.Subscriber('/mag', Float64MultiArray, self.mag_callback)
		self.gyro_sub = rospy.Subscriber('/gyro', Float64MultiArray, self.gyro_callback)
		self.accel_sub = rospy.Subscriber('/accel', Float64MultiArray, self.accel_callback)
	...
	def gyro_callback(self, msg):
		...
		self.sensor_gyro = np.array(msg.data)
	
	def accel_callback(self, msg):
		self.sensor_flags[2] = 1
		self.sensor_accel = np.array(msg.data)
